...
execute_creation() {
    ...
    if [ "$USE_PARALLEL" = "y" ]; then
        ...
        for i in $(seq 1 $COUNT); do
            ...
            # Progress update
            if [ $((i % 10)) -eq 0 ]; then
                echo -ne "\rGestartet: $i / $COUNT Jobs..."
            fi
        done

        # Wait for all remaining jobs to complete
        echo -ne "\rWarte auf Abschluss aller Jobs..."
        ...
        # Count results (this is approximate since we don't collect all outputs)
        success=$COUNT
        failed=0
        print_info "Alle parallelen Jobs abgeschlossen"
    else
